...
set -e
rm -rf /tmp/pub && mkdir /tmp/pub && cp out/shops.json out/next_shifts.json out/todo.enc.json /tmp/pub/ 2>/dev/null || cp out/shops.json out/next_shifts.json /tmp/pub/
cd /tmp/pub
git init -q -b data
